...
class DocGiaService {
  ...
  /**
   * Tìm kiếm theo Tên hoặc Mã Độc Giả
  ...
  async search(query) {
    return await this.find({
      $or: [
        { tenDocGia: { $regex: new RegExp(query), $options: "i" } },
        { hoLot: { $regex: new RegExp(query), $options: "i" } },
        { maDocGia: { $regex: new RegExp(query), $options: "i" } },
        { email: { $regex: new RegExp(query), $options: "i" } },
        { dienThoai: { $regex: new RegExp(query), $options: "i" } },
      ],
    });
  }
